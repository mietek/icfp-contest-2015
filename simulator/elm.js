...
Elm.Simulator.make = function (_elm) {
   ...
   $Simulator$Input = Elm.Simulator.Input.make(_elm),
   $Simulator$Output = Elm.Simulator.Output.make(_elm),
   $Simulator$Random = Elm.Simulator.Random.make(_elm),
   $Simulator$Simulation = Elm.Simulator.Simulation.make(_elm),
   $Simulator$Unit = Elm.Simulator.Unit.make(_elm),
   $Time = Elm.Time.make(_elm),
   $Util$List = Elm.Util.List.make(_elm),
   $Util$Maybe = Elm.Util.Maybe.make(_elm);
   var onChangeSendAction = F2(function (address,
   valueToAction) {
   ...
   var selectRandomUnits = F3(function (n,
   availableUnits,
   seed) {
      return function () {
         var randoms = $Basics.fst(A2($Simulator$Random.take,
         n,
         $Simulator$Random.init(seed)));
         ...
         $List.length(availableUnits)),
         randoms);
         var _raw = $Util$Maybe.sequence(A2($List.map,
         $Util$List.at(availableUnits),
         indexes)),
         $ = _raw.ctor === "Just" ? _raw : _U.badCase($moduleName,
         "on line 66, column 7 to 72"),
         list = $._0;
         return list;
      }();
   });
   var toSimulation = function (model) {
      return function () {
         var andThen = $Maybe.andThen;
         return A2(andThen,
         $Simulator$Input.parse(model.input),
         function (input) {
            return A2(andThen,
            $Simulator$Output.parse(model.output),
            function (outputs) {
               return A2(andThen,
               $List.head(input.units),
               function (firstUnit) {
                  return A2(andThen,
                  $List.head(outputs),
                  function (output) {
                     ...
                     function (seed) {
                        return A2(andThen,
                        $Simulator$Command.decodeList(output.solution),
                        function (commands) {
                           return function () {
                              var units = A3(selectRandomUnits,
                              input.sourceLength,
                              input.units,
                              output.seed);
                              var attributes = {_: {}
                                               ...
                                               ,units: units
                                               ,width: input.width};
                              return $Maybe.Just($Simulator$Simulation.init(attributes));
                           }();
                        });
                     });
                  });
      ...
      return function () {
         ...
         {case "InputChange":
            ...
            return _U.replace([["output"
              ...
              model);
            case "Submit":
            return _U.replace([["simulation"
                               ,toSimulation(model)]],
              model);
            case "Tick":
            return _U.replace([["simulation"
                               ,A2($Maybe.map,
                               $Simulator$Simulation.update,
                               model.simulation)]],
              model);}
         _U.badCase($moduleName,
         "between lines 99 and 110");
      }();
   });
   var submittedOutput = Elm.Native.Port.make(_elm).inbound("submittedOutput",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
      v);
   });
   var submittedInput = Elm.Native.Port.make(_elm).inbound("submittedInput",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
      v);
   ...
   var OutputChange = function (a) {
      return {ctor: "OutputChange"
             ,_0: a};
   };
   var InputChange = function (a) {
      return {ctor: "InputChange"
             ,_0: a};
   };
   ...
   model) {
      return function () {
         var simulationView = function () {
            var _v3 = model.simulation;
            switch (_v3.ctor)
            {case "Just":
               return A2($Simulator$Simulation.view,
                 address,
                 _v3._0);
               case "Nothing":
               return $Html.text("...");}
         ...
         _L.fromArray([A2($Html.input,
                      _L.fromArray([$Html$Attributes.placeholder("Program input")
                                   ,$Html$Attributes.value(model.input)
                                   ,A2(onChangeSendAction,
                                   address,
                                   InputChange)]),
                      _L.fromArray([]))
                      ,A2($Html.input,
                      _L.fromArray([$Html$Attributes.placeholder("Program output")
      ...
      F2(function (_v7,model) {
         return function () {
            ...
            {case "Just": return A2(update,
                 ...
                 model);}
            _U.badCase($moduleName,
            "on line 42, column 34 to 53");
         }();
      }),
      A2(update,Submit,initialModel),
      A2($Signal.merge,
      actions.signal,
      ticks));
   ...
   _elm.Simulator.values = {_op: _op
                           ...
                           ,selectRandomUnits: selectRandomUnits
...
Elm.Simulator = Elm.Simulator || {};
Elm.Simulator.Cell = Elm.Simulator.Cell || {};
Elm.Simulator.Cell.make = function (_elm) {
   "use strict";
   _elm.Simulator = _elm.Simulator || {};
   _elm.Simulator.Cell = _elm.Simulator.Cell || {};
   if (_elm.Simulator.Cell.values)
   return _elm.Simulator.Cell.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Simulator.Cell",
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Simulator$Command = Elm.Simulator.Command.make(_elm);
   var fromInputCell = function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "InputCell":
            switch (_v0._0.ctor)
              {case "_Tuple2":
                 return {ctor: "_Tuple2"
                        ,_0: _v0._0._0 - $Basics.floor($Basics.toFloat(_v0._0._1) / 2)
                        ,_1: _v0._0._1};}
              break;}
         _U.badCase($moduleName,
         "on line 43, column 4 to 32");
      }();
   };
   var rotateCell = F3(function (rotation,
   pivot,
   cell) {
      return function () {
         var $ = pivot,
   ...
   var y = $Basics.snd;
   var x = $Basics.fst;
   var InputCell = function (a) {
      return {ctor: "InputCell"
             ,_0: a};
   };
   var toInputCell = function (_v11) {
      return function () {
         switch (_v11.ctor)
         {case "_Tuple2":
            return InputCell({ctor: "_Tuple2"
                             ,_0: _v11._0 + $Basics.floor($Basics.toFloat(_v11._1) / 2)
                             ,_1: _v11._1});}
         _U.badCase($moduleName,
         "on line 47, column 3 to 42");
      }();
   };
   var toInputCellCoords = function (cell) {
      return function () {
         var unpack = function (_v15) {
            return function () {
               switch (_v15.ctor)
               {case "InputCell":
                  return _v15._0;}
               _U.badCase($moduleName,
               "on line 50, column 39 to 45");
            }();
         };
         return unpack(toInputCell(cell));
      }();
   };
   _elm.Simulator.Cell.values = {_op: _op
                                ,InputCell: InputCell
                                ,x: x
                                ,y: y
                                ,moveCell: moveCell
                                ,rotateCell: rotateCell
                                ,fromInputCell: fromInputCell
                                ,toInputCell: toInputCell
                                ,toInputCellCoords: toInputCellCoords};
   return _elm.Simulator.Cell.values;
};
Elm.Simulator = Elm.Simulator || {};
Elm.Simulator.Command = Elm.Simulator.Command || {};
Elm.Simulator.Command.make = function (_elm) {
   "use strict";
   _elm.Simulator = _elm.Simulator || {};
   _elm.Simulator.Command = _elm.Simulator.Command || {};
   if (_elm.Simulator.Command.values)
   return _elm.Simulator.Command.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Simulator.Command",
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   var decode = function (code) {
      return function () {
         var mapping = _L.fromArray([{ctor: "_Tuple2"
                                     ,_0: _L.fromArray([_U.chr("p")
                                                       ,_U.chr("\'")
                                                       ,_U.chr("!")
                                                       ,_U.chr(".")
                                                       ,_U.chr("0")
                                    ...
                                    ,{ctor: "_Tuple2"
                                     ,_0: _L.fromArray([_U.chr("k")
                                                       ...
                                                       ,_U.chr("x")])
                                     ,_1: Rotate(CCW)}]);
         var maybeMappingRow = A2($Util$List.find,
         function (_v0) {
            return function () {
               switch (_v0.ctor)
               {case "_Tuple2":
                  return A2($List.member,
                    ...
                    _v0._0);}
               _U.badCase($moduleName,
               "on line 27, column 43 to 65");
            }();
         },
         mapping);
         var maybeCommand = A2($Maybe.map,
         $Basics.snd,
         maybeMappingRow);
         return maybeCommand;
      }();
   };
   var decodeList = function (codes) {
      return $Util$Maybe.sequence(A2($List.map,
      decode,
      codes));
   };
   _elm.Simulator.Command.values = {_op: _op
                                   ,E: E
                                   ,W: W
                                   ,SE: SE
                                   ,SW: SW
...
Elm.Simulator = Elm.Simulator || {};
Elm.Simulator.Input = Elm.Simulator.Input || {};
Elm.Simulator.Input.make = function (_elm) {
   "use strict";
   _elm.Simulator = _elm.Simulator || {};
   _elm.Simulator.Input = _elm.Simulator.Input || {};
   if (_elm.Simulator.Input.values)
   return _elm.Simulator.Input.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Simulator.Input",
   $Basics = Elm.Basics.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var decoder = A8($Json$Decode.object7,
   Input,
   A2($Json$Decode._op[":="],
   "id",
   $Json$Decode.$int),
   ...
   var parse = function (inputString) {
      return $Result.toMaybe(A2($Json$Decode.decodeString,
      decoder,
      inputString));
   };
   _elm.Simulator.Input.values = {_op: _op
                                 ,Input: Input
                                 ,parse: parse
                                 ,decoder: decoder
                                 ,cell: cell
...
Elm.Simulator = Elm.Simulator || {};
Elm.Simulator.Output = Elm.Simulator.Output || {};
Elm.Simulator.Output.make = function (_elm) {
   "use strict";
   _elm.Simulator = _elm.Simulator || {};
   _elm.Simulator.Output = _elm.Simulator.Output || {};
   if (_elm.Simulator.Output.values)
   return _elm.Simulator.Output.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Simulator.Output",
   $Basics = Elm.Basics.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   $Simulator$Command = Elm.Simulator.Command.make(_elm),
   $String = Elm.String.make(_elm);
   var commandCodes = A2($Json$Decode.map,
   $String.toList,
   $Json$Decode.string);
   var Output = F4(function (a,
   b,
   c,
   d) {
      return {_: {}
   ...
   var entry = A5($Json$Decode.object4,
   Output,
   A2($Json$Decode._op[":="],
   "problemId",
   $Json$Decode.$int),
   ...
   "solution",
   commandCodes));
   var decoder = $Json$Decode.list(entry);
   var parse = function (outputString) {
      return $Result.toMaybe(A2($Json$Decode.decodeString,
      decoder,
      outputString));
   };
   _elm.Simulator.Output.values = {_op: _op
                                  ,Output: Output
                                  ,parse: parse
                                  ,decoder: decoder
                                  ,entry: entry
...
Elm.Simulator = Elm.Simulator || {};
Elm.Simulator.Random = Elm.Simulator.Random || {};
Elm.Simulator.Random.make = function (_elm) {
   "use strict";
   _elm.Simulator = _elm.Simulator || {};
   _elm.Simulator.Random = _elm.Simulator.Random || {};
   if (_elm.Simulator.Random.values)
   return _elm.Simulator.Random.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Simulator.Random",
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var LCGen = function (a) {
      return {ctor: "LCGen",_0: a};
   };
   var init = function (s) {
      return LCGen(s);
   };
   var next = function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "LCGen":
            return function () {
                 var r = A2($Basics._op["%"],
                 _v0._0 / Math.pow(2,16) | 0,
                 Math.pow(2,15));
                 var $ = {ctor: "_Tuple4"
                         ,_0: 129749
                         ,_1: 7
                 ...
                 m3 = $._2,
                 m4 = $._3;
                 var seedTimesMultiplier = A2($Basics._op["%"],
                 A2($Basics._op["%"],
                 A2($Basics._op["%"],
                 A2($Basics._op["%"],
                 _v0._0 * m1,
                 ...
                 Math.pow(2,32));
                 var increment = 12345;
                 var seed$ = A2($Basics._op["%"],
                 seedTimesMultiplier + increment,
                 Math.pow(2,32));
                 return {ctor: "_Tuple2"
                        ,_0: r
                        ,_1: LCGen(seed$)};
         ...
         _U.badCase($moduleName,
         "between lines 21 and 37");
      }();
   };
   var take = F2(function (n,g) {
      return _U.cmp(n,
      0) > 0 ? function () {
         var $ = next(g),
         r = $._0,
         ...
         g$$ = $._1;
         return {ctor: "_Tuple2"
                ,_0: A2($List._op["::"],r,tail)
                ,_1: g$$};
      }() : {ctor: "_Tuple2"
            ,_0: _L.fromArray([])
            ,_1: g};
   });
   _elm.Simulator.Random.values = {_op: _op
                                  ,init: init
                                  ,next: next
                                  ,take: take};
   return _elm.Simulator.Random.values;
...
Elm.Simulator = Elm.Simulator || {};
Elm.Simulator.Simulation = Elm.Simulator.Simulation || {};
Elm.Simulator.Simulation.make = function (_elm) {
   "use strict";
   _elm.Simulator = _elm.Simulator || {};
   _elm.Simulator.Simulation = _elm.Simulator.Simulation || {};
   if (_elm.Simulator.Simulation.values)
   return _elm.Simulator.Simulation.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Simulator.Simulation",
   $Basics = Elm.Basics.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Util$List = Elm.Util.List.make(_elm);
   var cellCssPosition = function (cell) {
      return function () {
         var cellSpacing = 2;
         var cellSize = 56;
         var w = cellSize + cellSpacing;
         var $ = function () {
            var _v0 = $Simulator$Cell.toInputCell(cell);
            switch (_v0.ctor)
            {case "InputCell":
               switch (_v0._0.ctor)
                 {case "_Tuple2":
         ...
         x = $._0,
         y = $._1;
         var xPos = x * w + (_U.eq(A2($Basics._op["%"],
         y,
         2),
         1) ? $Basics.round(0.5 * w) : 0);
         var yPos = y * (cellSize - 6);
         return _L.fromArray([{ctor: "_Tuple2"
                              ,_0: "left"
                              ,_1: A2($Basics._op["++"],
                              $Basics.toString(xPos),
                              "px")}
                             ,{ctor: "_Tuple2"
                              ,_0: "top"
                              ,_1: A2($Basics._op["++"],
                              $Basics.toString(yPos),
                              "px")}]);
      }();
   };
   var cellView = F2(function (color,
   cell) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class(A2($Basics._op["++"],
                   "hexagon hexagon-56 hexagon-",
                   color))
                   ,$Html$Attributes.style(cellCssPosition(cell))]),
      _L.fromArray([A2($Html.span,
      _L.fromArray([]),
      _L.fromArray([$Html.text(A2($Basics._op["++"],
      $Basics.toString($Simulator$Cell.x(cell)),
      A2($Basics._op["++"],
      ",",
      $Basics.toString($Simulator$Cell.y(cell)))))]))]));
   });
   var pivotView = function (pivot) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("hexagon hexagon-56 hexagon-dark-gray")
                   ,$Html$Attributes.style(A2($Basics._op["++"],
                   cellCssPosition(pivot),
   ...
   var unitView = function (unit) {
      ...
      cellView("violet"),
      unit.cells),
      _L.fromArray([pivotView(unit.pivot)]));
   };
   var statusView = function (model) {
      ...
      _L.fromArray([$Html.text(A2($Basics._op["++"],
                   ...
                   "Step: ",
                   A2($Basics._op["++"],
                   $Basics.toString(model.step),
                   " ")))
                   ,$Html.text(model.gameOver ? "Game over!" : "")]));
   };
   var filterFullRows = function (model) {
      return function () {
         var cellsInRow = function (row) {
            return A2($List.filter,
            function (_v4) {
               return function () {
                  switch (_v4.ctor)
                  {case "_Tuple2":
                     return _U.eq(_v4._1,row);}
                  _U.badCase($moduleName,
                  "on line 126, column 31 to 39");
               }();
            },
            model.filled);
         };
         var isFullRow = function (row) {
            return _U.eq($List.length(cellsInRow(row)),
            model.width);
         };
         var rows = A2($Util$List.range,
         0,
         model.height - 1);
         return A2($List.filter,
         isFullRow,
   ...
   var updateScore = function (model) {
      return function () {
         var unitSize = $List.length(model.unit.cells);
         var rowsCleared = $List.length(filterFullRows(model));
         var points = unitSize + 50 * (1 + rowsCleared) * rowsCleared;
         var bonus = _U.cmp(model.prevRowsCleared,
         1) > 0 ? $Basics.floor($Basics.toFloat((model.prevRowsCleared - 1) * points) / 10) : 0;
         return _U.replace([["score"
                            ,model.score + points + bonus]
                           ,["prevRowsCleared"
                            ,rowsCleared]],
         model);
      }();
   };
   var clearFullRow = F2(function (model,
   row) {
      return function () {
         var shiftDownCell = function (_v8) {
            return function () {
               switch (_v8.ctor)
               {case "_Tuple2":
                  return A2($Simulator$Cell.moveCell,
                    $Util.isEven(_v8._1) ? $Simulator$Command.SE : $Simulator$Command.SW,
                    {ctor: "_Tuple2"
                    ,_0: _v8._0
                    ,_1: _v8._1});}
               _U.badCase($moduleName,
               "on line 109, column 7 to 51");
            }();
         };
         var shiftDownCellsAboveRow = F2(function (row,
         cells) {
            return A2($List.map,
            function (_v12) {
               return function () {
                  switch (_v12.ctor)
                  {case "_Tuple2":
                     return _U.cmp(_v12._1,
                       row) < 0 ? shiftDownCell({ctor: "_Tuple2"
                                                ,_0: _v12._0
                                                ,_1: _v12._1}) : {ctor: "_Tuple2"
                                                                 ,_0: _v12._0
                                                                 ,_1: _v12._1};}
                  _U.badCase($moduleName,
                  "on line 112, column 28 to 76");
               }();
            },
            cells);
         });
         var skipCellsFromRow = F2(function (row,
         cells) {
            return A2($List.filter,
            function (_v16) {
               return function () {
                  switch (_v16.ctor)
                  {case "_Tuple2":
                     return !_U.eq(_v16._1,row);}
                  _U.badCase($moduleName,
                  "on line 106, column 31 to 39");
               }();
            },
            cells);
         });
         var newFilledCells = function ($) {
            return shiftDownCellsAboveRow(row)(skipCellsFromRow(row)($));
         }(model.filled);
         return _U.replace([["filled"
                            ,newFilledCells]],
         model);
      }();
   });
   var clearFullRows = function (model) {
      return function () {
         var firstFullRow = $List.head(filterFullRows(model));
         return function () {
            switch (firstFullRow.ctor)
            {case "Just":
               return clearFullRows(A2(clearFullRow,
                 model,
                 firstFullRow._0));
   ...
   var lockUnitCells = function (model) {
      return _U.replace([["filled"
                         ,A2($Basics._op["++"],
                         model.unit.cells,
                         model.filled)]],
   ...
   var isValidPosition = function (model) {
      return function () {
         var cellWithinBounds = function (cell) {
            return function () {
               var coords = $Simulator$Cell.toInputCellCoords(cell);
               return _U.cmp($Basics.fst(coords),
               0) > -1 && (_U.cmp($Basics.fst(coords),
               model.width) < 0 && _U.cmp($Basics.snd(coords),
               model.height) < 0);
            }();
         };
         var withinBounds = function (unit) {
            return A2($List.all,
            cellWithinBounds,
            unit.cells);
         };
         var colliding = function (unit) {
            return A2($List.any,
            function (cell) {
               return A2($List.member,
               cell,
         ...
         return withinBounds(model.unit) && $Basics.not(colliding(model.unit));
      }();
   };
   var spawnNextUnit = function (model) {
      return function () {
         var _v22 = model.units;
         switch (_v22.ctor)
         {case "::": return function () {
                 var newModel = _U.replace([["unit"
                                            ,_v22._0]
                                           ,["units",_v22._1]],
                 model);
                 return isValidPosition(newModel) ? newModel : _U.replace([["gameOver"
                                                                           ,true]],
                 newModel);
              }();
            case "[]":
            return _U.replace([["gameOver"
                               ,true]],
              model);}
         _U.badCase($moduleName,
   ...
   var applyCommand = F2(function (command,
   model) {
      return model.gameOver ? model : function () {
         var newModel = _U.replace([["unit"
                                    ,A2($Simulator$Unit.commandUnit,
                                    command,
                                    model.unit)]
                                   ,["step",model.step + 1]],
         model);
         return isValidPosition(newModel) ? newModel : function ($) {
            return spawnNextUnit(clearFullRows(updateScore(lockUnitCells($))));
         }(model);
      }();
   });
   var update = function (model) {
      return function () {
         var _v25 = model.commands;
         switch (_v25.ctor)
         {case "::":
            return A2(applyCommand,
              _v25._0,
              _U.replace([["commands"
                          ,_v25._1]],
              model));
   ...
   var allCells = function (model) {
      return function () {
         var pairs = A2($Util$List.product,
         A2($Util$List.range,
         0,
         model.width - 1),
         A2($Util$List.range,
         ...
         return A2($List.map,
         function ($) {
            return $Simulator$Cell.fromInputCell($Simulator$Cell.InputCell($));
         },
         pairs);
      }();
   };
   var boardView = function (model) {
      return function () {
         var unitCellsViews = unitView(model.unit);
         var filledCellsViews = A2($List.map,
         cellView("yellow"),
         model.filled);
         var allCellsViews = A2($List.map,
         cellView("gray"),
         allCells(model));
         return A2($Html.div,
   ...
   model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([statusView(model)
                   ,boardView(model)]));
   });
   var init = function (attributes) {
      return function () {
         var centeredUnits = A2($List.map,
         $Simulator$Unit.centerUnit(attributes.width),
         attributes.units);
         var _raw = centeredUnits,
         $ = _raw.ctor === "::" ? _raw : _U.badCase($moduleName,
         "on line 41, column 23 to 36"),
         unit = $._0,
         units = $._1;
         return {_: {}
   ...
   var InitAttributes = F5(function (a,
   b,
   c,
   d,
   e) {
   ...
   _elm.Simulator.Simulation.values = {_op: _op
                                      ,init: init
                                      ,update: update
                                      ,view: view
                                      ,Model: Model};
...
Elm.Simulator = Elm.Simulator || {};
Elm.Simulator.Unit = Elm.Simulator.Unit || {};
Elm.Simulator.Unit.make = function (_elm) {
   "use strict";
   _elm.Simulator = _elm.Simulator || {};
   _elm.Simulator.Unit = _elm.Simulator.Unit || {};
   if (_elm.Simulator.Unit.values)
   return _elm.Simulator.Unit.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Simulator.Unit",
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
         ...
         var toCoords = function (_v0) {
            return function () {
               ...
               {case "InputCell":
                  return _v0._0;}
               _U.badCase($moduleName,
               "on line 52, column 30 to 36");
            }();
         };
         var inputCells = A2($List.map,
         $Simulator$Cell.toInputCell,
         unit.cells);
         var xs = A2($List.map,
         function ($) {
            return $Basics.fst(toCoords($));
         },
         inputCells);
         var _ = {ctor: "_Tuple2"
                 ,_0: $List.minimum(xs)
   ...
   var fromInputCellsUnit = function (inputUnit) {
      return _U.replace([["cells"
                         ,A2($List.map,
                         $Simulator$Cell.fromInputCell,
                         inputUnit.cells)]
                        ,["pivot"
                         ,$Simulator$Cell.fromInputCell(inputUnit.pivot)]],
      inputUnit);
   };
   var rotateUnit = F2(function (rotation,
   unit) {
      return _U.replace([["cells"
                         ,A2($List.map,
                         A2($Simulator$Cell.rotateCell,
                         rotation,
                         unit.pivot),
                         unit.cells)]],
      unit);
   });
   var moveUnit = F2(function (move,
   unit) {
      return _U.replace([["cells"
                         ,A2($List.map,
                         $Simulator$Cell.moveCell(move),
                         unit.cells)]
                        ,["pivot"
                         ,A2($Simulator$Cell.moveCell,
                         move,
                         unit.pivot)]],
      unit);
   });
   var commandUnit = function (command) {
      return function () {
         switch (command.ctor)
         {case "Move":
            return moveUnit(command._0);
            case "Rotate":
            return rotateUnit(command._0);}
         _U.badCase($moduleName,
   ...
   _elm.Simulator.Unit.values = {_op: _op
                                ,Unit: Unit
                                ,moveUnit: moveUnit
...
Elm.Util.List.make = function (_elm) {
   ...
   if (_elm.Util.List.values)
   return _elm.Util.List.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Util.List",
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   var find = function (predicate) {
      return function ($) {
         return $List.head($List.filter(predicate)($));
      };
   };
   ...
   var at = F2(function (list,
   index) {
      return $List.head(A2($List.drop,
      index,
      list));
   });
   _elm.Util.List.values = {_op: _op
                           ,at: at
                           ,product: product
                           ,range: range
                           ,find: find};
...
Elm.Util = Elm.Util || {};
Elm.Util.Maybe = Elm.Util.Maybe || {};
Elm.Util.Maybe.make = function (_elm) {
   "use strict";
   _elm.Util = _elm.Util || {};
   _elm.Util.Maybe = _elm.Util.Maybe || {};
   if (_elm.Util.Maybe.values)
   return _elm.Util.Maybe.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Util.Maybe",
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   _elm.Util.Maybe.values = {_op: _op
                            ,sequence: sequence};
   return _elm.Util.Maybe.values;
};
